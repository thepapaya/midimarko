...
int cur_note = -1; //The MIDI note value being played
int is_muted = 0; // Whether or not the system is muted
...
void loop() {
  ...
  if (note > 0 && !is_muted)
  {
    noteOff(0, cur_note, 60);
    noteOn(0, note, 60);
    cur_note = note;
  }
  else if (note > 0 && is_muted)
  {
    // Record the new note value to be played after unmuting
    cur_note = note;
  }
  else if (note == 0 && !is_muted)
  {
    // Turn the note off and begin silence
    noteOff(0, cur_note, 60);
    is_muted = 1;
  }
  else if (note == 0 && is_muted)
  {
    // Turn the note on to the new value of cur_note
    is_muted = 0;
    noteOn(0, cur_note, 60);
  }
